...
namespace GraphProcessor
{
    ...
    public class BaseGraph : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        /// <param name="inputMemberName">input field name</param>
        /// <param name="outputNode">output node</param>
        /// <param name="outputMemberName">output field name</param>
        public void Disconnect(BaseNode inputNode, string inputMemberName, BaseNode outputNode, string outputMemberName)
        {
            edges.RemoveAll(r =>
            {
                bool remove = r.inputNode == inputNode
                && r.outputNode == outputNode
                && r.outputMemberName == outputMemberName
                && r.inputMemberName == inputMemberName;
...
#pragma warning restore CS0618
        ...
        void DestroyBrokenGraphElements()
        {
            edges.RemoveAll(e => e.inputNode == null
                || e.outputNode == null
                || string.IsNullOrEmpty(e.outputMemberName)
                || string.IsNullOrEmpty(e.inputMemberName)
            );
            nodes.RemoveAll(n => n == null);
        }
