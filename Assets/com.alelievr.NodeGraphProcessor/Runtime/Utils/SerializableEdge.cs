...
namespace GraphProcessor
{
    ...
    public class SerializableEdge : ISerializationCallbackReceiver
    {
        ...
        public static SerializableEdge CreateNewEdge(BaseGraph graph, NodePort inputPort, NodePort outputPort)
        {
            ...
            edge.inputMemberName = inputPort.memberName;
            edge.outputNode = outputPort.owner;
            edge.outputMemberName = outputPort.memberName;
        ...
        public void Deserialize()
        {
            ...
            inputPort = inputNode.GetPort(inputMemberName, inputPortIdentifier);
            outputPort = outputNode.GetPort(outputMemberName, outputPortIdentifier);
        }

        public override string ToString() => $"{outputNode.name}:{outputPort.memberName} -> {inputNode.name}:{inputPort.memberName}";
    }
}
